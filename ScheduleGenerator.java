...
import model.TeacherScheduleList;
...
public class ScheduleGenerator {
    ...
    TeacherScheduleList teacherScheduleList = new TeacherScheduleList(); 
    public static void main(String[] args) {

        ScheduleGenerator scheduleGenerator = new ScheduleGenerator();
        scheduleGenerator.readSchedule();
        scheduleGenerator.readDutyList();
        // next step = go through scheudle list and assign day 1 and day 2 duty to the teacher
        scheduleGenerator.setSpare();
        scheduleGenerator.calculateSlots();
    ...
    public void calculateSlots() {
        ...
        for (DutyTime dutyTime : dutyTimeList.getScheduleMap().values()) {
            ...
            for (int i = 0; i< dayType.length; i++) {
                ...
                teacherSchedule.setTeacher(allocateTeacher(dutyTime, dayType[i], i));
    ...
    public String allocateTeacher(DutyTime dutyTime, int dayType, int dayNumber) {
        if (dayType == 1) {
            for (Schedule schedule : scheduleList.getScheduleMap().values()) {
                ...
                if (dutyTime.getSpareDay1().contains(schedule.getSpare()) && dayNumber != schedule.getAllocated()) {
        ...
        if (dayType == 2) {
            for (Schedule schedule : scheduleList.getScheduleMap().values()) {
                ...
                if(dutyTime.getSpareDay2().contains(schedule.getSpare()) && dayNumber != schedule.getAllocated()) {
                    schedule.setAllocated(dayNumber);
                    scheduleList.writeSchedule(schedule.getTeacher(), schedule, null);
                    return schedule.getTeacher();
                }
